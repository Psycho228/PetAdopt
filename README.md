...
приютов. Пользователь заполняет анкету, просматривает питомцев, отправляет
заявку и общается с приютом. Сотрудники приюта управляют питомцами и заявками
через Android-интерфейс или отдельную веб-панель.
...
- Kotlin 1.9.24, AGP 8.2.2, JDK 17;
...
Роли: `user`, `shelter`, `admin`. Статусы заявок: `pending`, `processing`,
`approved`, `rejected`.
