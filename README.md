...
приютов и публикации проверенных предложений от заводчиков. Каталог питомцев
и объявления питомников доступны без авторизации. Регистрация требуется при
отправке заявки, работе с личным профилем или входе в кабинет заводчика.
Сотрудники приюта управляют питомцами и заявками, а администраторы проверяют
профили заводчиков и объявления.

## Возможности

### Для будущих владельцев

- гостевой просмотр свайп-карточек, подробных анкет и каталога питомников;
- регистрация и вход через Supabase Auth;
- предложение регистрации при отправке заявки;
- автоматический переход нового пользователя к опроснику после регистрации;
...
- отдельная регистрация и авторизация на экране входа;
- профиль питомника с контактами, городом и породами;
- создание и редактирование объявлений о продаже;
- загрузка до шести фотографий, выбор главного снимка и удаление отдельных фото;
- свайп-галерея фотографий в опубликованном объявлении;
...
- загрузка нескольких фотографий в S3-совместимое хранилище;
...
Новый профиль питомника создается в `breeder_profiles` со статусом `pending`.
До завершения модерации маршрутизация определяет кабинет заводчика по наличию
профиля, даже если роль `breeder` еще не назначена. После подтверждения триггер
из `add_breeder_marketplace.sql` назначает роль автоматически. Модерация
профилей и объявлений доступна администратору в разделе веб-панели
`/breeders`.
...
Значения `VITE_*` встраиваются в JavaScript во время сборки. После замены
Supabase `ANON_KEY` недостаточно изменить `.env` на сервере: панель нужно
пересобрать и повторно загрузить содержимое `dist/`. После развертывания
проверьте новый hash имени файла `assets/index-*.js` и обновите кэш браузера.
...
Android-приложение использует `S3StorageRepository` как активную реализацию
`StorageRepository`. Перед загрузкой `ImageUploadProcessor` считывает EXIF,
физически исправляет ориентацию, ограничивает большую сторону до 1024 px и
сохраняет результат как JPEG. Это единый путь для фотографий приюта и
питомника. Старые изображения, загруженные до появления нормализации EXIF,
нужно выбрать повторно: восстановить удаленную метку ориентации невозможно.

Главное фото объявления хранится в `sale_listings.photo_url`, остальные — в
`sale_listings.additional_photos`. Форма питомника ограничивает галерею шестью
изображениями.
...
Автоматических тестов пока мало. Кроме стандартных примеров есть проверки
парсинга ответа GigaChat и восстановления русскоязычного mojibake.
